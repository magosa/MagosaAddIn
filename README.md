...
PowerPointでの図形操作を効率化するVSTOアドインです。図形の整列・分割・配置・選択補助・ハンドル調整・レイヤー管理・ナンバリング・置き換え・サイズ調整・配列複製・テーマカラー生成・テキスト一括編集・スタイルライブラリ・**画像倍率同期**・**画像色編集**・**選択順序変更**機能を提供します。
...
- **選択オブジェクトスタック**: 複数の選択状態を保存・復元
  ...
  - ワンクリックで過去の選択状態を復元
  - 削除された図形は自動的に無効として表示
- **選択順序変更** *(v1.1.1.0 新機能)*: 選択図形の処理順序をダイアログで変更
  - ListView で選択図形を一覧表示（図形名・種類・Z 順）
  - ↑↓ ボタンで並び替え（先頭/末尾では無効化）
  - 後続アクション選択：選択状態を維持 / レイヤー順に適用 / スタックに保存
...
### 🎨 画像色編集機能 *(v1.1.1.0 新機能)*
- **色調編集**: 選択した画像の明るさ・コントラスト・色相・彩度をスライダーで調整
- **カラーライズ**: Photoshop「色彩の統一」相当の HSL チャンネル置換（H・S を統一し L を保持）
- **色調変換（排他）**: グレースケール・セピア・白黒（閾値スライダー付き）
- **リアルタイムプレビュー**: 300ms デバウンスによる変更前/後のプレビュー表示
- **HSL カラーピッカー**: 色相・彩度・輝度スライダーと HEX 入力に対応したカラー選択ダイアログ
- **GradientBar スライダー**: 全スライダーがグラデーション背景付きのカスタムコントロール

### 🖼️ 画像倍率同期機能 *(v1.0.10 新機能)*
- **スケール同期**: 2つの画像の実寸法測定に基づき、画像②を画像①と同じ実寸スケールにリサイズ
- **クリックまたは数値入力**: プレビュー上のクリックで測定区間を指定、または座標を直接入力
- **解像度補正**: クリップボードビットマップの解像度差（高/低解像度スキャン等）を自動補正
- **測定方向指定**: 自由（斜め）・水平のみ・垂直のみの3モード
- **単位対応**: mm・cm単位で実寸法を指定
- **プレビューズーム**: +/−ボタンまたはマウスホイールで25%〜800%ズーム
- **位置保持オプション**: 中心位置保持 または 左上位置保持を選択
- **スケール係数表示**: リアルタイムでスケール係数・変換後サイズを計算表示
...
│   ├── ImageScaler.cs            # 画像倍率同期機能 ★v1.0.10
│   ├── ImageColorEditor.cs       # 画像色編集処理パイプライン ★v1.1.1.0
│   ├── ImageColorSettings.cs     # 色編集パラメータ DTO ★v1.1.1.0
│   ├── ShapeOrderManager.cs      # 選択順序変更機能 ★v1.1.1.0
...
│   └── DataModels.cs             # 共通データクラス（MeasurementMode, ImageInfo 等）
├── UI/                            # ユーザーインターフェース層
...
│       ├── ImageScaleSyncDialog.cs         # 画像倍率同期設定 ★v1.0.10
...
│       ├── ImageColorEditDialog.cs         # 画像色編集ダイアログ ★v1.1.1.0
│       ├── ColorPickerDialog.cs            # HSL カラーピッカー ★v1.1.1.0
│       ├── GradientBar.cs                  # 共通グラデーションスライダーコントロール ★v1.1.1.0
│       ├── SelectionOrderDialog.cs         # 選択順序変更ダイアログ ★v1.1.1.0
...
#### 概要
Magick.NET を使用して選択した画像の色調をダイアログで編集し、PowerPoint 図形に適用する機能。
リアルタイムプレビューで仕上がりを確認しながら調整できます。

#### 調整パラメータ
| パラメータ | 範囲 | 説明 |
|-----------|------|------|
| 明るさ | -100 〜 +100 | 画像全体の輝度調整 |
| コントラスト | -100 〜 +100 | 明暗差の調整 |
| 色相 | -180° 〜 +180° | 色環上の色相シフト |
| 彩度 | -100 〜 +100 | 鮮やかさの調整 |
| カラーライズ強度 | 0 〜 100 | 指定色の彩度強度 |
...
| グレースケール | グレースケール変換 |
| セピア | セピアトーン変換 |
| 白黒 | 二値化（閾値: 0〜100%、スライダー付き） |

#### カラーライズ（Photoshop「色彩の統一」相当）
- 各ピクセルの H（色相）と S（彩度）を指定色で統一し、L（輝度）は元画像を保持
- ImageMagick の HSL カラースペースでチャンネルを直接置換する方式を採用
- 色調変換モードが指定されている場合はカラーライズをスキップ（排他制御）

#### 実装クラス
| クラス | ファイル | 役割 |
|--------|---------|------|
| `ImageColorEditor` | Core/ImageColorEditor.cs | Magick.NET 処理パイプライン・PowerPoint 図形置換 |
| `ImageColorSettings` | Core/ImageColorSettings.cs | 色編集パラメータ DTO |
| `ImageColorEditDialog` | UI/Dialogs/ImageColorEditDialog.cs | 編集ダイアログ（リアルタイムプレビュー付き） |
| `GradientBar` | UI/Dialogs/GradientBar.cs | 共通グラデーションスライダーコントロール |
| `ColorPickerDialog` | UI/Dialogs/ColorPickerDialog.cs | HSL カラーピッカーダイアログ |

### 🎯 選択順序変更機能（v1.1.1.0）

#### 概要
PowerPoint の `ShapeRange` は Z 順でインデックスが付与されるため、ユーザーが意図した処理順序と異なる場合がある。
この機能で選択図形の論理的な並び順を変更し、後続のレイヤー操作・スタック保存に反映できます。

#### 操作フロー
1. スライド上で 2 個以上の図形を選択
2. 「選択順序変更」ボタンをクリック
3. ListView で図形一覧を確認（初期順: Z 順）
4. ↑/↓ ボタンで並び替え
5. 後続アクションを選択して OK
...
| アクション | 説明 |
|-----------|------|
| 選択状態を維持 | 並び順の確認のみ（何も変更しない） |
| レイヤー順に適用して閉じる | 並び順をレイヤー（Z 順）に反映 |
| スタックに保存して閉じる | 並び順でスタックに保存 |

#### 実装クラス
| クラス | ファイル | 役割 |
|--------|---------|------|
| `ShapeOrderManager` | Core/ShapeOrderManager.cs | レイヤー適用・スタック保存ロジック |
| `SelectionOrderDialog` | UI/Dialogs/SelectionOrderDialog.cs | 図形並び替えダイアログ |

### 🖼️ 画像倍率同期機能（v1.0.10）

#### 概要
異なる解像度やスキャン設定で作成された2枚の画像を、同じ実寸スケール（mm/px）に揃えるリサイズ機能。
基準画像①と対象画像②それぞれで「実際の長さが分かる区間」を指定することで、スケール係数を自動計算します。

#### 計算式
$$F = \frac{\text{ratio}_2}{\text{ratio}_1} \times \frac{Px_2^{total} \times W_1}{Px_1^{total} \times W_2}$$

- $\text{ratio}_n$: 実寸法 / 測定ピクセル長（mm/px）
- $Px_n^{total}$: クリップボードビットマップの総ピクセル幅（解像度補正用）
- $W_n$: スライド上の表示幅（pt）

クリップボードがスクリーン解像度を返す場合、補正係数は1.0になり `ratio2/ratio1` と等価になります。
原寸解像度（高/低解像度スキャン等）の場合も同じ式で正しく動作します。

#### 入力項目
| 項目 | 説明 |
|------|------|
| 画像選択 | スライド内の画像オブジェクトをコンボボックスで選択 |
| 入力方式 | クリック指定（プレビュー上でクリック）または数値入力 |
| 測定方向 | 自由（斜め距離）・水平のみ・垂直のみ |
| 起点/終点座標 | 測定区間の始点・終点をピクセル座標で指定 |
| 実寸法 | 測定区間の実際の長さ（mm または cm） |
| プレビューズーム | 25%〜800%（ボタンまたはマウスホイール） |

#### スケール係数表示
- ピクセル長、倍率（mm/px）をリアルタイム計算・表示
- 計算後の画像②サイズをプレビュー

#### 実行オプション
| オプション | 説明 |
|------|------|
| 中心位置を保持する | リサイズ後も画像②の中心座標を維持 |
| 左上位置を保持する | リサイズ後も画像②の左上座標を維持 |

#### 実装クラス
| クラス | ファイル | 役割 |
|--------|---------|------|
| `ImageScaler` | Core/ImageScaler.cs | スケール係数計算・リサイズ適用・バリデーション |
| `ImageScaleSyncDialog` | UI/Dialogs/ImageScaleSyncDialog.cs | ダイアログUI（横並びレイアウト 1100×735px） |
| `MeasurementMode` | Core/DataModels.cs | 測定方向列挙型（Free/HorizontalOnly/VerticalOnly） |
...
| グループ | 機能 |
|----------|------|
| 選択補助 | 同一書式選択・スタックに追加・スタック復元・スタック数表示・**選択順序変更 ★v1.1.1.0** |
...
| 配列複製 | 円形配列・グリッド配列 |
| **画像操作** | **画像倍率同期 ★v1.0.10** / **色調編集 ★v1.1.1.0** |
| 基準整列 | 左端・右端・上端・下端・水平中央・垂直中央揃え |
| 隣接整列 | 各方向の隣接配置 |
...
| ImageColorEditDialog | 画像色編集（GradientBar スライダー・リアルタイムプレビュー）★v1.1.1.0 |
| ColorPickerDialog | HSL カラーピッカー（色相・彩度・輝度スライダー + HEX 入力）★v1.1.1.0 |
| SelectionOrderDialog | 選択順序変更（ListView + ↑↓ ボタン・後続アクション選択）★v1.1.1.0 |
| ImageScaleSyncDialog | 画像倍率同期（横並びレイアウト・プレビューズーム付き）★v1.0.10 |
...
### 画像倍率同期アルゴリズム（v1.0.10）
- 実寸法÷ピクセル長で各画像のスケール（mm/px）を計算
- ビットマップ解像度補正: スクリーン解像度・原寸解像度どちらのケースにも対応
- クリックベース座標入力: Zoom表示モードの座標逆変換で正確なピクセル座標を取得
...
| 画像倍率同期（最小画像数） | スライド内に2枚以上の画像オブジェクトが必要 |
| 画像倍率同期（サイズ範囲） | 1pt〜10000pt || 画像色編集（明るさ・コントラスト） | -100 〜 +100 |
| 画像色編集（色相） | -180° 〜 +180° |
| 画像色編集（彩度・カラーライズ強度） | -100 〜 +100 / 0 〜 100 |
| 画像色編集（白黒閾値） | 0 〜 100% |
| 選択順序変更（最小図形数） | 2個以上 || 最大調整ハンドル数 | 8個 |
| ナンバリング開始番号 | 0～999 |
| ナンバリング増分値 | -10～+10 |
| サイズ調整パーセント範囲 | 10%～500% |
| 配列個数範囲 | 2～100個 |
...
1. 色調編集したい画像図形を1つ選択
2. 「色調編集」ボタンをクリック
3. 左ペインのスライダーで明るさ・コントラスト・色相・彩度を調整
4. 色調変換（グレースケール・セピア・白黒）を使う場合はラジオボタンで選択
   - 白黒の場合は閾値スライダーで二値化レベルを調整
5. カラーライズを使う場合はチェックボックスをON → 「色を選択」でカラーピッカーを開き色を指定
   - ※ 色調変換とカラーライズは排他（どちらかのみ有効）
6. 右ペインの「変更後」プレビューで仕上がりを確認
7. 「OK」で図形に適用

### 選択順序変更の使い方
1. 順序を変更したい図形を2個以上選択
2. 「選択順序変更」ボタンをクリック
3. ListView に図形が Z 順で一覧表示される
4. 並び替えたい行を選択して「↑ 上へ」「↓ 下へ」で順序を変更
5. 後続アクションのラジオボタンで処理内容を選択
   - **選択状態を維持**: 順序確認のみ（スライドに変更なし）
   - **レイヤー順に適用して閉じる**: 並び順でレイヤー（Z 順）を更新
   - **スタックに保存して閉じる**: 並び順でスタックに保存
6. 「OK」で実行
...
1. スライド上に画像を2枚以上配置
2. 「画像倍率同期」ボタンをクリック
3. 画像①（基準）と画像②（対象）をそれぞれコンボボックスで選択
4. 各画像のプレビュー上で測定区間を2点クリック（起点→終点）
   - ズームボタンまたはマウスホイールで拡大表示可能
5. 実寸法（mm または cm）を入力
6. スケール係数が自動計算されることを確認
7. 位置保持オプション（中心/左上）を選択
8. 「実行」ボタンで画像②をリサイズ
...
- **画像倍率同期**: スライド内に2枚以上の画像オブジェクト（msoPicture）が必要
- **画像色編集**: 1枚の画像図形（msoPicture）が必要
- **選択順序変更**: 2個以上の図形が必要
