...
| 図形操作 | グリッド分割・レイヤー調整・自動ナンバリング・テーマカラー生成 |
...
- **🎯 選択オブジェクトスタック機能の追加**:
  ...
  - **リボンUI拡張**:
    ...
    - ItemsLoadingイベントでメニュー項目を動的生成
  - **Constants.cs**: スタック関連定数を追加
    - MIN_SHAPES_FOR_STACK = 1（最小図形数）
    - MAX_STACK_COUNT = 20（最大スタック数）

- **🐛 バグ修正**: 図形置き換えダイアログの継承設定
  - **問題**: チェックボックスの変数とラベルの割り当てが逆
    - スタイル継承のみをチェック → 何も変化しない
    - テキスト継承のみをチェック → スタイルとテキスト両方が継承される
  - **修正**: ShapeReplacementDialog.csのチェックボックス作成順序を修正
    - chkInheritStyleとchkInheritTextの作成順序を入れ替え
    - 各チェックボックスが正しい機能に対応するように変更

- **🔧 技術的改善**:
  - **動的メニュー実装**: リボンメニューを動的コントロールとして設定
  - **ShapeRange選択**: 複数図形の一括選択を正しく実装
  - **COM参照の有効性チェック**: 削除された図形を自動検出
